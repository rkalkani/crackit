...
public class Result_Screen extends Activity {
	...
	//private TextView tv_data; // for display answersheet
	...
	// Button
	private ImageButton btn_examhome; // for go to home screen

	// private ScrollView sv_resultscreen;
	...
	private ArrayList<String> ArrayList_SkipAns = null; // Skipped question
	...
	// String
	private String string_percentage; // store percentage
	private String date; // store date
	private String time; // store time

	// Double
	private double percentage;

	// Int
	private int rightans; // total right answers
	private int wrongans; // total wrong answers
	private int skipans; // total skip questions
	private int database; // use as flag for insert result in to database
							// also used for count no of try to insert result
							// here maximum two try is taken for insert result
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		btn_examhome.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				if (database == 0) {
					// ExamTab_GroupActivity.group.resetExamTab_GroupActivity();
					// ResultTab_GroupActivity.group.resetResultTab_GroupActivity();
					finish();
				}
	...
	private void initData() {
		...
		// TextView
		// tv_heading = (TextView) findViewById(R.id.tv_resultecreen_heading);
		//tv_data = (TextView) findViewById(R.id.tv_resultscreen_data);
		...
		// Scroll
		// sv_resultscreen = (ScrollView) findViewById(R.id.sv_resultscreen1);
		...
		protected void onPostExecute(Boolean result) {
			...
			if (result == true) {
				...
				for (int i = 0; i < ArrayList_SkipAns.size(); i++) {
					//data = data + ArrayList_SkipAns.get(i) + "\n\n";
					List_data.add(ArrayList_SkipAns.get(i));
				}
				ArrayList_SkipAns.clear();

				// set answer sheet to screen
				//tv_data.setText(data);
				
				Answersheet_Adapter mAnswersheet_Adapter = new Answersheet_Adapter((ArrayList<String>) List_data, context);
